 ...
 *
 * Deactivates an active listing (pauses it)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Deactivate Listing] Request:', { listingId })
    ...
    // Check if listing can be deactivated (must be ACTIVE)
    if (listing.status !== 'ACTIVE') {
      return NextResponse.json(
        { error: `Cannot deactivate ${listing.status} listing` },
        { status: 400 }
      )
    }

    // Deactivate listing on StockX
    const operation = await StockxListingsService.deactivateListing(listingId)
