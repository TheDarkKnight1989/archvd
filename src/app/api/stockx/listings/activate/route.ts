 ...
 *
 * Activates an inactive/paused listing
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Activate Listing] Request:', { listingId })
    ...
    // Check if listing can be activated (must be INACTIVE or EXPIRED)
    ...
    if (listing.status === 'DELETED' || listing.status === 'MATCHED') {
      return NextResponse.json(
        { error: `Cannot activate ${listing.status} listing` },
        { status: 400 }
      )
    }

    // Activate listing on StockX
    const operation = await StockxListingsService.activateListing(listingId)
