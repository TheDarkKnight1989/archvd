...
/**
 * Hook for fetching KPI statistics
 */
export function useKPIStats(userId: string | undefined) {
  ...
  useEffect(() => {
    ...
    const fetchKPIs = async () => {
      try {
        ...
        setData({ totalItems, inStock, sold, totalValue });
        setError(null);
      } catch (err: any) {
    ...
    const fetchBreakdown = async () => {
      try {
        const { data: items, error: fetchError } = await supabase
          ...
          .eq('user_id', userId)
          .eq('status', 'in_stock'); // Only in-stock items for brand breakdown
    ...
    const fetchBreakdown = async () => {
      try {
        const { data: items, error: fetchError } = await supabase
          ...
          .eq('user_id', userId)
          .eq('status', 'in_stock'); // Only in-stock items for size breakdown
        ...
        console.error('Failed to fetch size breakdown:', err);
        setError(err.message);
      } finally {
        setLoading(false);
      }
      ...
      try {
        ...
        let tableRows: TableRow[] = (items || []).map((item: InventoryItem) => {
          ...
          return {
            ...
            market,
            marketSource,
            marketUpdatedAt: (item as any).market_updated_at || null,
            pl,
            plPct,
          };
        });
