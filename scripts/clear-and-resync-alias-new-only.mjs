...
/**
 * Clear ALL Alias market snapshots and re-sync with NEW condition priority
 */
...
async function clearAndResync() {
  console.log('\n=== CLEARING AND RE-SYNCING ALIAS DATA (NEW CONDITION ONLY) ===\n')

  // Step 1: Clear ALL existing snapshots
  console.log('Step 1: Clearing all existing Alias market snapshots...')
  const { error: deleteError } = await supabase
    .from('alias_market_snapshots')
    .delete()
    .neq('catalog_id', '')  // Delete all rows

  if (deleteError) {
    console.error('Error clearing snapshots:', deleteError)
    return
  }
  console.log('✓ Cleared all existing snapshots\n')

  // Step 2: Get all unique catalog IDs
  console.log('Step 2: Fetching catalog IDs to sync...')
  ...
  console.log(`✓ Found ${catalogIds.length} unique catalog IDs\n`)

  // Step 3: Re-sync with NEW condition priority
  console.log('Step 3: Re-syncing with NEW condition priority...\n')
  ...
  for (const catalogId of catalogIds) {
    ...
    try {
      ...
      // Log condition distribution for this catalog
      const selectedVariants = Array.from(variantsBySize.values())
      const newCount = selectedVariants.filter(v => v.product_condition === 'PRODUCT_CONDITION_NEW').length
      const newDefectCount = selectedVariants.filter(v => v.product_condition === 'PRODUCT_CONDITION_NEW_WITH_DEFECTS').length
      const usedCount = selectedVariants.filter(v => v.product_condition === 'PRODUCT_CONDITION_USED').length

      console.log(`  Selected ${selectedVariants.length} variants: ${newCount} NEW, ${newDefectCount} NEW_WITH_DEFECTS, ${usedCount} USED`)

      const snapshotTime = new Date().toISOString()
      const snapshots = selectedVariants.map(v => ({
  ...
  console.log('\n✅ All Alias data now uses NEW condition priority!')
}
