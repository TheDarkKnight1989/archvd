...
/**
 * Inventory V4 - Alias Bulk Sync
 * Sync multiple catalog IDs in sequence with progress tracking
 *
 * Usage:
 *   # From file
 *   npx tsx scripts/inventory-v4-alias-sync-bulk.ts catalog-ids.txt
 *   npx tsx scripts/inventory-v4-alias-sync-bulk.ts catalog-ids.json
 *
 *   # From args
 *   npx tsx scripts/inventory-v4-alias-sync-bulk.ts dunk-low-black-white-dd1391-100 jordan-1-high-...
 *
 * Features:
...
// ============================================================================
// Utility: Read Catalog IDs from File or Args
// ============================================================================

function readCatalogIds(input: string[]): string[] {
  if (input.length === 0) {
    throw new Error('No catalog IDs provided. Usage: npx tsx scripts/inventory-v4-alias-sync-bulk.ts <file|id1 id2 ...>');
  }
  ...
  // Treat as individual catalog IDs
  return input;
}
...
async function isProductSynced(catalogId: string): Promise<boolean> {
...
function saveResults(summary: BulkSyncSummary): string {
  ...
  const filename = `bulk-sync-alias-${timestamp}.json`;
...
async function bulkSync(catalogIds: string[]): Promise<BulkSyncSummary> {
  ...
  for (let i = 0; i < catalogIds.length; i++) {
    ...
    const avgDuration = durations.length > 0
      ...
      : 10000; // Default 10s estimate (faster than StockX due to fewer API calls)

    // Show progress
    renderProgress(i, catalogIds.length, summary.synced, summary.skipped, summary.failed, avgDuration);

    try {
      // Check if already synced (if enabled)
      if (SKIP_ALREADY_SYNCED && await isProductSynced(catalogId)) {
      ...
      // Sync the product
      const result = await fullSyncAliasProductByCatalogId(catalogId);
      const duration = Date.now() - itemStartTime;
  ...
  // Final progress (100%)
  renderProgress(catalogIds.length, catalogIds.length, summary.synced, summary.skipped, summary.failed, 0);
...
function displaySummary(summary: BulkSyncSummary): void {
  ...
  // Overall stats
  console.log('📈 Overall:');
  console.log(`  Total Catalog IDs:  ${summary.total}`);
  ...
  const totalSales = summary.results
    .filter(r => r.result?.success)
    .reduce((sum, r) => sum + (r.result?.counts.salesRecordsInserted || 0), 0);
  ...
  if (summary.failed > 0) {
    ...
    summary.results
      ...
      .forEach((r, idx) => {
        console.log(`  ${idx + 1}. ${r.catalogId}`);
        if (r.error) {
          console.log(`     Error: ${r.error}`);
        } else if (r.result) {
          r.result.errors.forEach(err => {
            const location = err.region ? `Region ${err.region}` : err.size ? `Size ${err.size}` : '';
            console.log(`     [${err.stage}] ${location} ${err.error}`);
          });
        }
      });
    console.log('');
  ...
  const avgDuration = summary.results.length > 0
    ...
    : 0;
  console.log(`⏱️  Average Duration: ${formatDuration(avgDuration)} per product`);
  console.log('');

  // Next steps
  console.log('🔍 Next Steps:');
  console.log('');
  console.log('  1. Refresh materialized view:');
  console.log('     REFRESH MATERIALIZED VIEW CONCURRENTLY inventory_v4_alias_market_latest;');
  console.log('');
  console.log('  2. Verify data in database:');
  console.log('     SELECT COUNT(*) FROM inventory_v4_alias_products;');
  console.log('     SELECT COUNT(*) FROM inventory_v4_alias_variants;');
  console.log('     SELECT COUNT(*) FROM inventory_v4_alias_market_data;');
  console.log('');
}
...
async function main() {
  ...
  console.log('🔄 INVENTORY V4 - ALIAS BULK SYNC');
  console.log('='.repeat(80));
  console.log('');
  ...
  try {
    catalogIds = readCatalogIds(args);
  } catch (error) {
    console.error('❌ Error:', (error as Error).message);
    console.error('');
    console.error('Usage:');
    console.error('  npx tsx scripts/inventory-v4-alias-sync-bulk.ts catalog-ids.txt');
    console.error('  npx tsx scripts/inventory-v4-alias-sync-bulk.ts dunk-low-... jordan-1-...');
    console.error('');
    process.exit(1);
  }

  console.log(`📋 Catalog IDs to sync: ${catalogIds.length}`);
  console.log(`⚙️  Skip already-synced: ${SKIP_ALREADY_SYNCED ? 'Yes' : 'No'}`);
  console.log(`💾 Save results: ${SAVE_RESULTS ? 'Yes' : 'No'}`);
  console.log(`🌍 Regions: UK 🇬🇧 → EU 🇪🇺 → US 🇺🇸`);
  console.log('');
  console.log('='.repeat(80));
  console.log('');

  // Run bulk sync
  const summary = await bulkSync(catalogIds);
