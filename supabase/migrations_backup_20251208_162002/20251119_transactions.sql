...
CREATE INDEX IF NOT EXISTS ix_transactions_occurred_at ON transactions (occurred_at DESC);
CREATE INDEX IF NOT EXISTS ix_transactions_inventory ON transactions (inventory_id) WHERE inventory_id IS NOT NULL;
...
-- 5) Views for convenient filtering
CREATE OR REPLACE VIEW v_transactions_sales AS
  SELECT * FROM transactions WHERE type = 'sale';

CREATE OR REPLACE VIEW v_transactions_purchases AS
