-- =============================================================================
-- Sprint: Alpha Polish - Watchlist Alerts + Activity Feed (FIXED)
-- Migration: Watchlist price alerts and portfolio activity logging
-- Date: 2025-11-13
-- Fix: Removed currency column references (doesn't exist in watchlist_items)
-- =============================================================================

-- Drop existing objects if migration was partially applied
...
-- A3. Function to check and refresh watchlist alerts (FIXED)
...
BEGIN
  ...
  LOOP
    ...
    IF v_item.sku LIKE 'PKMN-%' THEN
      -- Pokémon: use tcg_latest_prices (GBP only)
      ...
      LIMIT 1;
    ELSE
      -- Sneakers: use sneaker_latest_prices (default to UK9 if size not specified, GBP only)
      SELECT median_price INTO v_current_price
...
DROP POLICY IF EXISTS "Users can view own activity log" ON public.portfolio_activity_log;
...
DROP POLICY IF EXISTS "Users can insert own activity log" ON public.portfolio_activity_log;
...
DROP POLICY IF EXISTS "Service role can manage activity log" ON public.portfolio_activity_log;
...
-- B7. Enhanced alert logging via trigger on watchlist_items (FIXED)
...
BEGIN
  ...
  IF NEW.last_triggered_at IS NOT NULL AND
    ...
    -- Get user_id from parent watchlist
    ...
    IF NEW.sku LIKE 'PKMN-%' THEN
      ...
      FROM tcg_latest_prices
      WHERE sku = NEW.sku AND currency = 'GBP'
      ORDER BY as_of DESC
      LIMIT 1;
    ELSE
      ...
      FROM sneaker_latest_prices
      WHERE sku = NEW.sku
        AND size = COALESCE(NEW.size, 'UK9')
        AND currency = 'GBP'
      ORDER BY as_of DESC
      LIMIT 1;
    END IF;
    ...
    PERFORM log_portfolio_activity(
      v_user_id,
      'alert',
      'Price alert: ' || COALESCE(v_product_name, NEW.sku) || ' now £' || COALESCE(v_current_price, 0)::text ||
        ' (target: £' || NEW.target_price::text || ')',
      NEW.sku,
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration 20251113_watchlist_alerts_and_activity (FIXED) completed successfully';
