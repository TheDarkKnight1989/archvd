-- ============================================================================
-- ADD STOCKX CURRENCY PARAMETER TO UNIFIED MARKET DATA RPC
-- Date: 2025-12-13
-- Purpose: Filter StockX market data by currency (GBP, EUR, USD)
-- Matches Alias region support for consistent multi-region pricing
-- ============================================================================

-- ============================================================================
-- DROP OLD FUNCTION SIGNATURES (required because parameter list changed)
-- ============================================================================

DROP FUNCTION IF EXISTS get_unified_market_data(TEXT, TEXT, BOOLEAN);
DROP FUNCTION IF EXISTS get_unified_market_data_batch(TEXT[], TEXT[], TEXT, BOOLEAN, INT);

-- ============================================================================
-- FUNCTION: get_unified_market_data (updated with p_stockx_currency)
-- ============================================================================
...
CREATE OR REPLACE FUNCTION get_unified_market_data(
  ...
  p_stockx_currency TEXT DEFAULT 'GBP'  -- NEW: Filter StockX by currency
)
RETURNS TABLE (
  -- Size info
  size_display TEXT,
...
AS $$
  ...
  stockx_data AS (
    -- Get StockX variants + market data for specified currency
    SELECT
    ...
    LEFT JOIN inventory_v4_stockx_market_data sm
      ...
      AND sm.currency_code = p_stockx_currency  -- Filter by currency
...
-- ============================================================================
-- FUNCTION: get_unified_market_data_batch (updated with p_stockx_currency)
-- ============================================================================
...
-- ============================================================================
-- INDEX: StockX market data by currency
-- ============================================================================

CREATE INDEX IF NOT EXISTS idx_stockx_market_data_variant_currency
  ON inventory_v4_stockx_market_data(stockx_variant_id, currency_code);
...
COMMENT ON FUNCTION get_unified_market_data IS
  'Get combined StockX + Alias market data for a single SKU. Supports currency/region filtering.';

COMMENT ON FUNCTION get_unified_market_data_batch IS
  'Get combined StockX + Alias market data for multiple SKUs + sizes. Supports currency/region filtering.';
